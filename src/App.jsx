      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
        {visible.map((s) => (
          <div key={s.id} style={{ background: SURFACE, borderRadius: 16, padding: 14 }}>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12, color: MUTED, borderTop: "1px solid #232323", paddingTop: 10 }}>
              <span>Shares: <EditableNumber value={s.shares} onSave={(v) => update(s.id, "shares", v)} decimals={0} color={WHITE} fontSize={12} accent={accent} /></span>
              <span>Price: <EditableNumber value={s.price} onSave={(v) => update(s.id, "price", v)} prefix="$" color={WHITE} fontSize={12} accent={accent} /></span>
            </div>
          </div>
        ))}
        {!pro && <LockedCard accent={accent} />}
