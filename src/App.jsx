...
const CARD_TEMPLATES = {
  ...
  emerald: { label: "Emerald", bg: "#00A651", border: "#00C767", text: "#0A0A0A", sub: "#0B3D2A", chipBg: "#1A1A1A", chipBorder: "#000000" },
};
...
function CardFace({ card, username }) {
  const t = CARD_TEMPLATES[card.style] || CARD_TEMPLATES.obsidian;
  return (
    <div style={{ position: "relative", borderRadius: 20, padding: 24, height: 190, background: t.bg, border: `1px solid ${t.border}`, overflow: "hidden", flex: "0 0 100%", scrollSnapAlign: "center", boxSizing: "border-box" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
        <div style={{ width: 44, height: 34, borderRadius: 7, background: t.chipBg, position: "relative", overflow: "hidden", border: `1px solid ${t.chipBorder}` }}>
          <div style={{ position: "absolute", left: "50%", top: 0, bottom: 0, width: 1, background: t.chipBorder }} />
          <div style={{ position: "absolute", top: "33%", left: 0, right: 0, height: 1, background: t.chipBorder }} />
          <div style={{ position: "absolute", top: "66%", left: 0, right: 0, height: 1, background: t.chipBorder }} />
          <div style={{ position: "absolute", left: "22%", top: 0, bottom: 0, width: 1, background: t.chipBorder }} />
          <div style={{ position: "absolute", left: "78%", top: 0, bottom: 0, width: 1, background: t.chipBorder }} />
          <div style={{ position: "absolute", left: "30%", right: "30%", top: "22%", bottom: "22%", border: `1px solid ${t.chipBorder}`, borderRadius: 3 }} />
        </div>
        <Wifi size={20} color={t.sub} style={{ transform: "rotate(90deg)" }} />
      </div>
      <div className="num" style={{ marginTop: 26, fontSize: 20, letterSpacing: "0.14em", color: t.text, fontWeight: 500 }}>•••• •••• •••• {card.last4}</div>
      <div style={{ marginTop: 22, display: "flex", justifyContent: "space-between", alignItems: "flex-end" }}>
        <div>
          <div style={{ fontSize: 10, color: t.sub, marginBottom: 3, fontWeight: 600 }}>CARD HOLDER</div>
          <div style={{ fontSize: 13, color: t.text, fontWeight: 500 }}>{username.toUpperCase()}</div>
        </div>
        <div style={{ fontFamily: "'Space Grotesk', sans-serif", fontSize: 22, fontWeight: 800, fontStyle: "italic", color: t.text, letterSpacing: "-0.01em" }}>VISA</div>
      </div>
    </div>
  );
}

function CardCarousel({ cards, username, accent }) {
  ...
  return (
    <div style={{ marginBottom: 14 }} onTouchStart={(e) => e.stopPropagation()} onTouchEnd={(e) => e.stopPropagation()}>
      <div
        ref={scrollerRef}
        onScroll={handleScroll}
        style={{ display: "flex", overflowX: "auto", scrollSnapType: "x mandatory", gap: 12, scrollbarWidth: "none" }}
      >
        {cards.map((c) => <CardFace key={c.id} card={c} username={username} />)}
      </div>
      {cards.length > 1 && (
        <div style={{ display: "flex", justifyContent: "center", gap: 6, marginTop: 10 }}>
          {cards.map((c, i) => (
            <div key={c.id} style={{ width: 6, height: 6, borderRadius: "50%", background: i === active ? accent : "#333", transition: "background 0.2s ease" }} />
          ))}
        </div>
      )}
    </div>
...
function HomeView({ username, setUsername, balance, setBalance, cards, transactions, setTransactions, pro, accent, portfolioTotal, notify }) {
  ...
  const handleConfirm = (amount, name) => {
    if (modal === "add") {
      ...
      notify && notify("Funds added", `+$${fmt(amount)} added to your balance`);
    } else if (modal === "send") {
      ...
      notify && notify("Money sent", `$${fmt(capped)} sent${name ? ` to ${name}` : ""}`);
    } else if (modal === "request") {
      flashToast(`Request for $${fmt(amount)} sent${name ? ` to ${name}` : ""}`);
      notify && notify("Request sent", `$${fmt(amount)} requested${name ? ` from ${name}` : ""}`);
    }
    setModal(null);
  };
  ...
  return (
    <>
      ...
      <CardCarousel cards={cards} username={username} accent={accent} />

      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", background: SURFACE, borderRadius: 14, padding: "13px 16px", marginBottom: 22 }}>
        <div style={{ fontSize: 12, color: MUTED, fontWeight: 600 }}>Total assets (cash + crypto + stocks)</div>
        <div className="num" style={{ fontSize: 14.5, fontWeight: 700, color: accent }}>${fmt(portfolioTotal)}</div>
      </div>
...
function TxView({ transactions, setTransactions, pro, accent }) {
  ...
  const filtered = query.trim() ? transactions.filter((t) => t.label.toLowerCase().includes(query.trim().toLowerCase())) : transactions;
  ...
  return (
    <>
      ...
      <input placeholder="Search transactions" value={query} onChange={(e) => setQuery(e.target.value)}
        style={{ width: "100%", background: SURFACE, border: "1px solid #232323", borderRadius: 12, padding: "11px 14px", color: WHITE, fontSize: 13, marginBottom: 14, outline: "none", boxSizing: "border-box" }} />

      <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
        {filtered.length === 0 && (
          <div style={{ textAlign: "center", color: MUTED, fontSize: 12.5, padding: "20px 0" }}>No matching transactions</div>
        )}
        {filtered.map((item) => (
          <div key={item.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", background: SURFACE, borderRadius: 14, padding: "13px 14px" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
...
function SettingsView({ pro, setPro, theme, setTheme, accent, cards, setCards, pinEnabled, pin, biometricEnabled, onSavePin, onDisableLock, onToggleBiometric, alertsEnabled, onToggleAlerts, onReset }) {
  ...
  const handleAlertsToggle = async () => {
    ...
    if (typeof Notification !== "undefined" && Notification.permission !== "granted") {
      try {
        ...
        if (perm !== "granted") {
          setAlertMsg("Notifications not granted — you'll still get in-app alerts.");
          setTimeout(() => setAlertMsg(""), 2800);
        }
      } catch (e) {}
    }
    onToggleAlerts(true);
  ...
  const addCard = () => {
    ...
    setCards([...cards, { id: `card${Date.now()}`, last4: randomCardDigits(), style: "obsidian" }]);
  };
  const removeCard = (id) => {
    if (cards.length <= 1) return;
    setCards(cards.filter((c) => c.id !== id));
  };
  const setCardTemplate = (id, style) => setCards(cards.map((c) => (c.id === id ? { ...c, style } : c)));
  ...
  return (
    <>
      ...
      <div style={{ background: SURFACE, borderRadius: 16, padding: 18, marginBottom: 16 }}>
        ...
        {pro ? (
          <div style={{ fontSize: 12.5, color: MUTED, lineHeight: 1.5 }}>
            Pro unlocked — extra coins & stocks, live trading charts, custom accent colors, card templates, a second card, market alerts, transaction categories, and a minimized demo badge.
          </div>
        ) : (
          <>
      ...
      <div style={{ background: SURFACE, borderRadius: 16, padding: 18, marginBottom: 16 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
          <div style={{ fontSize: 14, fontWeight: 700, color: WHITE }}>Manage Cards</div>
          <div style={{ fontSize: 11, color: MUTED, fontWeight: 600 }}>{cards.length}/2</div>
        </div>
        {cards.map((c) => (
          <div key={c.id} style={{ marginBottom: 14, paddingBottom: 14, borderBottom: "1px solid #232323" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
              <div className="num" style={{ fontSize: 12.5, color: MUTED }}>•••• {c.last4}</div>
              {cards.length > 1 && (
                <button onClick={() => removeCard(c.id)} style={{ background: "none", border: "none", cursor: "pointer", color: "#F87171", fontSize: 11.5, fontWeight: 600 }}>
                  Remove
                </button>
              )}
            </div>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: 8 }}>
              {Object.entries(CARD_TEMPLATES).map(([key, t]) => {
                ...
                return (
                  <button key={key} onClick={() => !locked && setCardTemplate(c.id, key)} disabled={locked}
                    style={{ position: "relative", height: 46, borderRadius: 10, background: t.bg, border: c.style === key ? `2px solid ${accent}` : `1px solid ${t.border}`, cursor: locked ? "default" : "pointer", opacity: locked ? 0.4 : 1, display: "flex", alignItems: "center", justifyContent: "center", gap: 6 }}>
                    {locked && <Lock size={12} color={t.text} />}
                    <span style={{ fontSize: 11.5, fontWeight: 600, color: t.text }}>{t.label}</span>
                  </button>
                );
              })}
            </div>
        ...
        {cards.length < 2 && (
          pro ? (
            <button onClick={addCard} style={{ width: "100%", background: SURFACE_2, border: "1px dashed #3A3A3A", borderRadius: 10, padding: "11px 0", color: WHITE, fontWeight: 600, fontSize: 13, cursor: "pointer" }}>
              + Add a second card
            </button>
          ) : (
            <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 8, background: SURFACE_2, borderRadius: 10, padding: "11px 0", color: MUTED, fontSize: 12.5, fontWeight: 600 }}>
              <Lock size={13} /> Add a second card with Pro
            </div>
          )
        )}
      </div>
      ...
      <div style={{ background: SURFACE, borderRadius: 16, padding: 18, marginBottom: 16 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
          <div style={{ fontSize: 14, fontWeight: 700, color: WHITE }}>Market & Transaction Alerts</div>
          <div style={{ fontSize: 11.5, fontWeight: 700, color: alertsEnabled ? accent : MUTED }}>{alertsEnabled ? "ON" : "OFF"}</div>
        </div>
        <div style={{ fontSize: 12.5, color: MUTED, lineHeight: 1.5, marginBottom: 14 }}>
          Get alerts on transactions and occasional simulated market moves. Uses your browser's notifications when allowed, otherwise shows in-app banners — won't work if the app is wrapped as a standalone APK.
        </div>
        {pro ? (
          <button onClick={handleAlertsToggle} style={{ width: "100%", background: alertsEnabled ? accent : SURFACE_2, border: "1px solid #2A2A2A", borderRadius: 10, padding: "11px 0", color: alertsEnabled ? BLACK : WHITE, fontWeight: 700, fontSize: 13, cursor: "pointer" }}>
            {alertsEnabled ? "Alerts enabled" : "Enable alerts"}
          </button>
        ) : (
          <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 8, background: SURFACE_2, borderRadius: 10, padding: "11px 0", color: MUTED, fontSize: 12.5, fontWeight: 600 }}>
            <Lock size={13} /> Unlock Pro for alerts
          </div>
        )}
        {alertMsg && <div style={{ fontSize: 11.5, color: MUTED, marginTop: 8, textAlign: "center" }}>{alertMsg}</div>}
      </div>
...
export default function FinanceDashboard() {
  ...
  const [cards, setCards] = useState(
    saved?.cards ?? [{ id: "card1", last4: randomCardDigits(), style: saved?.cardStyle ?? "obsidian" }]
  );
  ...
  const [alertsEnabled, setAlertsEnabled] = useState(saved?.alertsEnabled ?? false);
  ...
  // Prevent the browser's native pull-to-refresh gesture so the app feels native, not like a webpage.
  useEffect(() => {
  ...
  const notify = (title, body) => {
    ...
    setTimeout(() => setGlobalToast(""), 2600);
  };

  // Occasional simulated market-alert notifications when Pro + alerts are on.
  useEffect(() => {
    if (!pro || !alertsEnabled) return;
    const id = setInterval(() => {
      const pool = [...coins, ...stocks];
      if (pool.length === 0) return;
      const asset = pool[Math.floor(Math.random() * pool.length)];
      const name = asset.symbol || asset.ticker;
      const change = (Math.random() * 6 - 2).toFixed(1);
      notify("Market alert", `${name} is ${change >= 0 ? "up" : "down"} ${Math.abs(change)}% today`);
    }, 75000);
    return () => clearInterval(id);
    // eslint-disable-next-line
  }, [pro, alertsEnabled, coins, stocks]);
  ...
  const handleToggleAlerts = (val) => setAlertsEnabled(val);
  ...
  useEffect(() => {
    saveState({ username, balance, coins, stocks, transactions, pro, theme, cards, pinEnabled, pin, biometricEnabled, alertsEnabled });
  }, [username, balance, coins, stocks, transactions, pro, theme, cards, pinEnabled, pin, biometricEnabled, alertsEnabled]);
  ...
  const handleReset = () => {
    ...
    setPro(false);
    setTheme("green");
    setCards([{ id: "card1", last4: randomCardDigits(), style: "obsidian" }]);
    setAlertsEnabled(false);
  };

  const visibleCoins = pro ? coins : coins.slice(0, 3);
  const visibleStocks = pro ? stocks : stocks.slice(0, 3);
  const portfolioTotal =
    balance +
    visibleCoins.reduce((s, c) => s + c.amount * c.price, 0) +
    visibleStocks.reduce((s, x) => s + x.shares * x.price, 0);
  ...
  return (
    <div style={{ minHeight: "100vh", width: "100%", background: BLACK, fontFamily: "'Inter', -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif", color: WHITE, padding: "28px 18px 100px", boxSizing: "border-box", overscrollBehaviorY: "none" }}>
      <style>{`
      ...
      {globalToast && (
        <div style={{ position: "fixed", top: 18, left: "50%", transform: "translateX(-50%)", background: SURFACE_2, border: `1px solid ${accent}`, borderRadius: 12, padding: "10px 16px", fontSize: 12.5, color: WHITE, fontWeight: 600, zIndex: 70, maxWidth: "90%", textAlign: "center" }}>
          {globalToast}
        </div>
      )}
        ...
        <div key={tab} className={slideDir === "right" ? "tab-content-right" : "tab-content-left"}>
          ...
          {tab === "settings" && (
            <SettingsView
              ...
              pinEnabled={pinEnabled} pin={pin} biometricEnabled={biometricEnabled}
              onSavePin={handleSavePin} onDisableLock={handleDisableLock} onToggleBiometric={handleToggleBiometric}
              alertsEnabled={alertsEnabled} onToggleAlerts={handleToggleAlerts}
              onReset={handleReset}
            />
          )}
        </div>
